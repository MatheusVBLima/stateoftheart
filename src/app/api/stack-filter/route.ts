...
import { getStateOfTheArtByCategory, getStateOfTheArtFiltered } from "@/lib/queries";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const availableTechnologies = stateOfTheArtData.reduce((acc, categoryGroup) => {
      ...
      const categoryIndex = acc.findIndex(cat => cat.label === categoryGroup.category.name);
      if (categoryIndex !== -1) {
        acc[categoryIndex].technologies = categoryGroup.implementations.map(impl => ({
          name: impl.name,
          slug: impl.slug,
    ...
    return NextResponse.json({
      ...
      totalResults: filteredResults.reduce((total, group) => total + group.implementations.length, 0),
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      selectedTechnologies: technologies,
      results,
      totalResults: results.reduce((total, group) => total + group.implementations.length, 0),
    });
  } catch (error) {
